...
void TensorProcessorImpl_GPU::InitKernels(const char* a_filePath)
{
  ...
  std::string servPath = AlterShaderPath("shaders_gpu/z_matMulTranspose.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  matMulTransposeDSLayout = CreatematMulTransposeDSLayout();
  MakeComputePipelineAndLayout(servPath.c_str(), "main", kspec, matMulTransposeDSLayout, &matMulTransposeLayout, &matMulTransposePipeline);
}
